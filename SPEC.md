...
- **M5 — going outside, cheaply.** The Agent SDK's default is an open-ended
  ...
  - **M5.1 (built).** Connection registry, `catalog/connections.json`.
    ...
    grant and says so (#8 and #9 built, D-258 — #9's live proof owed on the
    first restart after it; #10's chips not yet, so the work bar names no
    doors until then). Two more are decided and not
