...
- **M5 — going outside, cheaply.** The Agent SDK's default is an open-ended
  ...
  - **M5.3 (built).** The deterministic router, `router.ts`, wrapped round
    ...
    | `oneshot` | a recipe matches strongly *and has landed once* — the method, on a 5-turn leash | 19c |
    | `agent` | everything else; a weak recipe match still lends its method | 52c |

    Both prices are measured over 148 jobs rather than projected, and they are
