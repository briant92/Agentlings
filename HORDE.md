...
| Suites | server 2,479 across 101 files · web 353 · typecheck clean (full run 2026-08-26, with #18) |
...
- `.agentlings/connections.json` is **empty**. Nothing a user added is
  waiting to be found — **including Buk** (#18, D-266): the adapter ships in
  the repo and is proven, but no connection is stored and no `BUK_API_KEY`
  is in `.env`. There is no Buk tenant on this machine, which is exactly
  what the ticket's last box is waiting on.
