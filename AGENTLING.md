...
| Search | `Grep` / `Glob` |
| Work on your code | A level's repo is a folder on this machine **or** a GitHub URL (D-275), and one reader — `repoTarget` in `packages/shared` — decides which, on the clone side, the promote side and the route that sets it. A folder is cloned with `git clone --local --no-hardlinks` into `sandbox/repo` and Approve replays `DIFF.patch` onto its working tree; a URL is cloned over https and Approve pushes `agentlings/<job>-<title>` and opens a pull request against the default branch, the branch and the pull request recorded separately because a token that can push and not open one leaves the work delivered and says so. The token is `GITHUB_TOKEN`, the same one the code-host door reads, handed to git as a transient `http.extraHeader` so the clone the session works in holds no credential. The folder half alone is **Not available hosted** (*repo work from a local path* — `existsSync(where.path)`, `server/src/index.ts`): it is a path on the operator's own machine, checked when it is set, and a container has none |
...
- **Headed, in a profile you signed into.** The runner launches a visible
  persistent browser context itself (`playwright-core`, on the channel
  `browserchannel.ts` measured — Edge here) on `--remote-debugging-port`,
  and Playwright MCP attaches over
...
(D-259), because the plan re-runs on every keystroke. **Live**, and proven
live: `scripts/prove-refusal-ui.mjs` ran 21/21 against the real server on
2026-08-26, watching the route reach the screen — the board's sentence
matching byte for byte, the rows drawn in the order the route sent them,
Start undisabled throughout, and `refusals.jsonl` unchanged at 904 bytes
across a run of nothing but refusing sentences.
