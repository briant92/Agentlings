...
/**
 * The scene interpreter: the one place scene data becomes drawing.
 *
 * It lived in `web/src/world` while the browser was the only thing that drew,
 * and moved here when it stopped being (D-109's precedent — the format came
 * first, the interpreter follows). A pack is now drawn by the live world, the
 * level card, the review preview and a headless renderer a session can run
 * inside its own sandbox; all four walk this code, so none of them can show a
 * pack a picture the others would not.
 */
